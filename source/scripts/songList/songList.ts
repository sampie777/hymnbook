...
export default class SongList {
  ...
  static getAllSongLists(): Realm.Results<SongListModel> {
    return Db.songs.realm().objects(SongListModel.schema.name) as Realm.Results<SongListModel>;
  }
  ...
  static createSongListIfNotExists(name: string) {
    ...
    Db.songs.realm().write(() => {
      // @ts-ignore
      Db.songs.realm().create(SongListModel.schema.name, new SongListModel(name));
    });
  }
